    ...
    cups_option_t *options)		/* I - Options */
{
  ...
  */
  ...
  {
    if (!_ppd_strcasecmp(optptr->name, "media") ||
        !_ppd_strcasecmp(optptr->name, "output-bin") ||
	!_ppd_strcasecmp(optptr->name, "output-mode") ||
	!_ppd_strcasecmp(optptr->name, "print-quality") ||
	!_ppd_strcasecmp(optptr->name, "sides"))
      continue;
    else if (!_ppd_strcasecmp(optptr->name, "resolution") ||
             !_ppd_strcasecmp(optptr->name, "printer-resolution"))
    {
	...
	*/
    ...
    else if (!_ppd_strcasecmp(optptr->name, "mirror"))
              ...
              const char   *choice)	/* I - Name of choice */
{
  ...
  if (choice[0] == '{' || !_ppd_strncasecmp(choice, "Custom.", 7))
    choice = "Custom";

  for (i = o->num_choices, c = o->choices; i > 0; i --, c ++)
    if (!_ppd_strcasecmp(c->choice, choice))
      return (c);
    ...
    */
    ...
    for (i = ppd->num_groups, group = ppd->groups; i > 0; i --, group ++)
      for (j = group->num_options, optptr = group->options;
           j > 0;
	   j --, optptr ++)
        if (!_ppd_strcasecmp(optptr->keyword, option))
	  return (optptr);
  ...
  */
  ...
  while (*s)
  {
    ...
    */
    ...
    ptr = choice;
    while (*s && !_ppd_isspace(*s) && ptr < (choice + sizeof(choice) - 1))
      *ptr++ = *s++;
             ...
             ppd_group_t *g)		/* I - Group to default */
{
  ...
  for (i = g->num_options, o = g->options; i > 0; i --, o ++)
    if (_ppd_strcasecmp(o->keyword, "PageRegion") != 0)
    ...
    */
        ...
        switch (cparam->type)
	{
	  ...
	  case PPD_CUSTOM_REAL :
	      cparam->current.custom_real = (float)_ppdStrScand(choice + 7,
	                                                         NULL, loc);
	      break;

	  case PPD_CUSTOM_POINTS :
	      cparam->current.custom_points = (float)_ppdStrScand(choice + 7,
	                                                           &units,
	                                                           loc);

              if (units)
	      {
        	if (!_ppd_strcasecmp(units, "cm"))
	          cparam->current.custom_points *= 72.0f / 2.54f;
        	else if (!_ppd_strcasecmp(units, "mm"))
	          cparam->current.custom_points *= 72.0f / 25.4f;
        	else if (!_ppd_strcasecmp(units, "m"))
	          cparam->current.custom_points *= 72.0f / 0.0254f;
        	else if (!_ppd_strcasecmp(units, "in"))
	          cparam->current.custom_points *= 72.0f;
        	else if (!_ppd_strcasecmp(units, "ft"))
  ...
  else if (choice[0] == '{')
  {
    ...
    */
	...
	switch (cparam->type)
	{
	  ...
	  case PPD_CUSTOM_INVCURVE :
	  case PPD_CUSTOM_REAL :
	      cparam->current.custom_real = (float)_ppdStrScand(val->value,
	                                                         NULL, loc);
	      break;

	  case PPD_CUSTOM_POINTS :
	      cparam->current.custom_points = (float)_ppdStrScand(val->value,
	                                                           &units,
	                                                           loc);
