...
static int	ppd_get_url(http_t **http, const char *url, char *name, size_t namesize);
...
static int	ppd_pwg_compare_finishings(_ppd_pwg_finishings_t *a, _ppd_pwg_finishings_t *b);
static int	ppd_pwg_compare_sizes(cups_size_t *a, cups_size_t *b);
static cups_size_t *ppd_pwg_copy_size(cups_size_t *size);
static void	ppd_pwg_free_finishings(_ppd_pwg_finishings_t *f);
    ...
    ipp_t      **attrs)			/* IO - IPP attributes, if any */
{
  ...
  _ppd_pwg_finishings_t *finishings;	/* Current finishings option */
  ...
  char		line[2048],		/* Current line */
		...
		*valueptr,		/* Pointer into value */
		...
		ppd_keyword[PPD_MAX_NAME];
					/* PPD keyword */
  _ppd_pwg_print_color_mode_t print_color_mode;
	...
	*/
    ...
    else if (!_ppd_strcasecmp(line, "Preset"))
    {
      ...
      */
      ...
      print_color_mode = (_ppd_pwg_print_color_mode_t)strtol(value, &valueptr, 10);
      print_quality    = (_ppd_pwg_print_quality_t)strtol(valueptr, &valueptr, 10);

      if (print_color_mode < _PPD_PWG_PRINT_COLOR_MODE_MONOCHROME ||
          print_color_mode >= _PPD_PWG_PRINT_COLOR_MODE_MAX ||
	  print_quality < _PPD_PWG_PRINT_QUALITY_DRAFT ||
	...
	pc->finishings =
      ...
      if ((finishings = calloc(1, sizeof(_ppd_pwg_finishings_t))) == NULL)
        goto create_error;
...
_ppdCacheCreateWithPPD(ppd_file_t *ppd)	/* I - PPD file */
{
  ...
  _ppd_pwg_print_color_mode_t pwg_print_color_mode;
					/* print-color-mode index */
  _ppd_pwg_print_quality_t	pwg_print_quality;
					/* print-quality index */
  ...
  _ppd_pwg_finishings_t	*finishings;	/* Current finishings value */
  ...
  */
  ...
  if ((ppd_attr = ppdFindAttr(ppd, "APPrinterPreset", NULL)) != NULL)
  {
	...
	*/
	...
	if (!strcmp(quality, "low"))
	  pwg_print_quality = _PPD_PWG_PRINT_QUALITY_DRAFT;
	else if (!strcmp(quality, "high"))
	  pwg_print_quality = _PPD_PWG_PRINT_QUALITY_HIGH;
	else
	  pwg_print_quality = _PPD_PWG_PRINT_QUALITY_NORMAL;
	...
	*/
	...
	if (pwg_print_quality != _PPD_PWG_PRINT_QUALITY_HIGH && graphicsType &&
	...
	*/
	...
	media_front_coating = cupsGetOption(
        ...
        if (pwg_print_quality != _PPD_PWG_PRINT_QUALITY_HIGH &&
	    media_front_coating &&
	...
	*/
        ...
        if (output_mode)
	{
	  if (!strcmp(output_mode, "monochrome"))
	    pwg_print_color_mode = _PPD_PWG_PRINT_COLOR_MODE_MONOCHROME;
	  else
	    pwg_print_color_mode = _PPD_PWG_PRINT_COLOR_MODE_COLOR;
	}
  ...
  if (!pc->num_presets[_PPD_PWG_PRINT_COLOR_MODE_MONOCHROME][_PPD_PWG_PRINT_QUALITY_DRAFT] &&
      !pc->num_presets[_PPD_PWG_PRINT_COLOR_MODE_MONOCHROME][_PPD_PWG_PRINT_QUALITY_NORMAL] &&
      !pc->num_presets[_PPD_PWG_PRINT_COLOR_MODE_MONOCHROME][_PPD_PWG_PRINT_QUALITY_HIGH])
  {
   /*
    ...
    */
    ...
    if (color_option && gray_choice)
    {
      ...
      */
      ...
      for (pwg_print_quality = _PPD_PWG_PRINT_QUALITY_DRAFT;
	   pwg_print_quality < _PPD_PWG_PRINT_QUALITY_MAX;
	   pwg_print_quality ++)
      {
	if (pc->num_presets[_PPD_PWG_PRINT_COLOR_MODE_COLOR][pwg_print_quality])
	{
	 /*
	  * Copy the color options...
	  */

	  num_options = pc->num_presets[_PPD_PWG_PRINT_COLOR_MODE_COLOR]
					[pwg_print_quality];
	  ...
	  if (options)
	  {
	    for (i = num_options, moption = options,
		     coption = pc->presets[_PPD_PWG_PRINT_COLOR_MODE_COLOR]
					   [pwg_print_quality];
		 i > 0;
		 i --, moption ++, coption ++)
	    {
	    ...
	    pc->num_presets[_PPD_PWG_PRINT_COLOR_MODE_MONOCHROME][pwg_print_quality] =
		num_options;
	    pc->presets[_PPD_PWG_PRINT_COLOR_MODE_MONOCHROME][pwg_print_quality] =
		options;
	  }
	}
	else if (pwg_print_quality != _PPD_PWG_PRINT_QUALITY_NORMAL)
	  continue;
	...
	*/
	...
	pc->num_presets[_PPD_PWG_PRINT_COLOR_MODE_MONOCHROME][pwg_print_quality] =
	    cupsAddOption(color_option, gray_choice,
			  pc->num_presets[_PPD_PWG_PRINT_COLOR_MODE_MONOCHROME]
					  [pwg_print_quality],
			  pc->presets[_PPD_PWG_PRINT_COLOR_MODE_MONOCHROME] +
			      pwg_print_quality);
      }
    }
  }
...
{
  int			i;		/* Looping var */
  _ppd_pwg_finishings_t	*f,		/* PWG finishings options */
			key;		/* Search key */
    ...
    int           *values)		/* O - Finishings values */
{
  ...
  _ppd_pwg_finishings_t	*f;		/* Current finishings option */
  ...
  */
  ...
  for (f = (_ppd_pwg_finishings_t *)cupsArrayFirst(pc->finishings);
       f;
    ...
    ipp_t        *attrs)		/* I - Attributes to write, if any */
{
  ...
  pwg_size_t		*size;		/* Current size */
  pwg_map_t		*map;		/* Current map */
  _ppd_pwg_finishings_t	*f;		/* Current finishing option */
  cups_option_t		*option;	/* Current option */
  ...
  */
  ...
  for (i = _PPD_PWG_PRINT_COLOR_MODE_MONOCHROME; i < _PPD_PWG_PRINT_COLOR_MODE_MAX; i ++)
    for (j = _PPD_PWG_PRINT_QUALITY_DRAFT; j < _PPD_PWG_PRINT_QUALITY_MAX; j ++)
		  ...
		  ipp_t  *response)	/* I - Get-Printer-Attributes response */
{
  ...
  */
  ...
  if (!response)
  {
    set_error(_("No IPP attributes."), 1);
    return (NULL);
  }
  ...
  */
  ...
  if (!_ppd_strncasecmp(make, "Hewlett Packard ", 16) ||
      !_ppd_strncasecmp(make, "Hewlett-Packard ", 16))
  {
    model = make + 16;
  ...
  if ((attr = ippFindAttribute(response, "printer-strings-uri", IPP_TAG_URI)) != NULL)
  {
    ...
    if (ppd_get_url(&http, ippGetString(attr, 0, NULL), stringsfile, sizeof(stringsfile)))
    {
      cupsFilePrintf(fp, "*cupsStringsURI: \"%s\"\n", ippGetString(attr, 0, NULL));

      strings = _ppdMessageLoad(stringsfile, _PPD_MESSAGE_STRINGS | _PPD_MESSAGE_UNQUOTE);
  ...
  */
  ...
  sizes = cupsArrayNew3((cups_array_func_t)ppd_pwg_compare_sizes, NULL, NULL, 0, (cups_acopy_func_t)ppd_pwg_copy_size, (cups_afree_func_t)free);
  ...
  if (cupsArrayCount(sizes) > 0)
  {
    ...
    */
    ...
    {
      _ppdStrFormatd(twidth, twidth + sizeof(twidth), size->width * 72.0 / 2540.0, loc);
      _ppdStrFormatd(tlength, tlength + sizeof(tlength), size->length * 72.0 / 2540.0, loc);
    ...
    for (size = (cups_size_t *)cupsArrayFirst(sizes); size; size = (cups_size_t *)cupsArrayNext(sizes))
    {
      _ppdStrFormatd(tleft, tleft + sizeof(tleft), size->left * 72.0 / 2540.0, loc);
      _ppdStrFormatd(tbottom, tbottom + sizeof(tbottom), size->bottom * 72.0 / 2540.0, loc);
      _ppdStrFormatd(tright, tright + sizeof(tright), (size->width - size->right) * 72.0 / 2540.0, loc);
      _ppdStrFormatd(ttop, ttop + sizeof(ttop), (size->length - size->top) * 72.0 / 2540.0, loc);
      _ppdStrFormatd(twidth, twidth + sizeof(twidth), size->width * 72.0 / 2540.0, loc);
      _ppdStrFormatd(tlength, tlength + sizeof(tlength), size->length * 72.0 / 2540.0, loc);
    ...
    */
    ...
    if (max_width > 0 && min_width < INT_MAX && max_length > 0 && min_length < INT_MAX)
    {
      ...
      _ppdStrFormatd(tleft, tleft + sizeof(tleft), left * 72.0 / 2540.0, loc);
      _ppdStrFormatd(tbottom, tbottom + sizeof(tbottom), bottom * 72.0 / 2540.0, loc);
      _ppdStrFormatd(tright, tright + sizeof(tright), right * 72.0 / 2540.0, loc);
      _ppdStrFormatd(ttop, ttop + sizeof(ttop), top * 72.0 / 2540.0, loc);

      cupsFilePrintf(fp, "*HWMargins: \"%s %s %s %s\"\n", tleft, tbottom, tright, ttop);

      _ppdStrFormatd(tmax, tmax + sizeof(tmax), max_width * 72.0 / 2540.0, loc);
      _ppdStrFormatd(tmin, tmin + sizeof(tmin), min_width * 72.0 / 2540.0, loc);
      cupsFilePrintf(fp, "*ParamCustomPageSize Width: 1 points %s %s\n", tmin, tmax);

      _ppdStrFormatd(tmax, tmax + sizeof(tmax), max_length * 72.0 / 2540.0, loc);
      _ppdStrFormatd(tmin, tmin + sizeof(tmin), min_length * 72.0 / 2540.0, loc);
  ...
  */
  ...
  if ((attr = ippFindAttribute(response, "media-source-supported", IPP_TAG_ZERO)) != NULL && (count = ippGetCount(attr)) > 1)
  {
        ...
        if (!strcmp(sources[j], keyword))
	{
	  ...
	  cupsFilePrintf(fp, "*%s.InputSlot %s/%s: \"\"\n", lang->language, ppdname, _ppdLangString(lang, msgid));
	  break;
	}
    }
  ...
  */
  ...
  if ((attr = ippFindAttribute(response, "media-type-supported", IPP_TAG_ZERO)) != NULL && (count = ippGetCount(attr)) > 1)
  {
	...
	if ((msgstr = _ppdMessageLookup(strings, msgid)) == msgid)
	  msgstr = keyword;
...
#define PRINTF_COLOROPTION(name,text,cspace,bpp) { cupsFilePrintf(fp, "*ColorModel %s: \"<</cupsColorSpace %d/cupsBitsPerColor %d/cupsColorOrder 0/cupsCompression 0>>setpagedevice\"\n", name, cspace, bpp); cupsFilePrintf(fp, "*%s.ColorModel %s/%s: \"\"\n", lang->language, name, _ppdLangString(lang, text)); }
  ...
  */
  ...
  if ((attr = ippFindAttribute(response, "sides-supported", IPP_TAG_KEYWORD)) != NULL && ippContainsString(attr, "two-sided-long-edge"))
  {
    cupsFilePrintf(fp, "*OpenUI *Duplex: PickOne\n"
		       ...
		       "*CloseUI: *Duplex\n", lang->language, _ppdLangString(lang, _("2-Sided Printing")), lang->language, _ppdLangString(lang, _("Off (1-Sided)")), lang->language, _ppdLangString(lang, _("Long-Edge (Portrait)")), lang->language, _ppdLangString(lang, _("Short-Edge (Landscape)")));
    ...
    if ((attr = ippFindAttribute(response, "urf-supported", IPP_TAG_KEYWORD)) != NULL)
    {
      for (i = 0, count = ippGetCount(attr); i < count; i ++)
      {
        ...
        if (!_ppd_strcasecmp(dm, "DM1"))
        {
          cupsFilePuts(fp, "*cupsBackSide: Normal\n");
          break;
        }
        else if (!_ppd_strcasecmp(dm, "DM2"))
        {
          cupsFilePuts(fp, "*cupsBackSide: Flipped\n");
          break;
        }
        else if (!_ppd_strcasecmp(dm, "DM3"))
        {
          cupsFilePuts(fp, "*cupsBackSide: Rotated\n");
          break;
        }
        else if (!_ppd_strcasecmp(dm, "DM4"))
        {
  ...
  if ((attr = ippFindAttribute(response, "finishings-supported", IPP_TAG_ENUM)) != NULL)
  {
    ...
    */
    ...
    if (ippContainsInteger(attr, IPP_FINISHINGS_BOOKLET_MAKER))
    {
      ...
      cupsFilePrintf(fp, "*%s.Translation Booklet/%s: \"\"\n", lang->language, _ppdLangString(lang, _("Booklet")));
  ...
  */
  ...
  else
  {
    ...
    if (ippContainsInteger(quality, IPP_QUALITY_DRAFT))
      cupsFilePrintf(fp, "*cupsPrintQuality Draft: \"<</HWResolution[%d %d]>>setpagedevice\"\n*%s.cupsPrintQuality Draft/%s: \"\"\n", xres, yres, lang->language, _ppdLangString(lang, _("Draft")));

    cupsFilePrintf(fp, "*cupsPrintQuality Normal: \"<</HWResolution[%d %d]>>setpagedevice\"\n*%s.cupsPrintQuality Normal/%s: \"\"\n", xres, yres, lang->language, _ppdLangString(lang, _("Normal")));

    if (ippContainsInteger(quality, IPP_QUALITY_HIGH))
      cupsFilePrintf(fp, "*cupsPrintQuality High: \"<</HWResolution[%d %d]>>setpagedevice\"\n*%s.cupsPrintQuality High/%s: \"\"\n", xres, yres, lang->language, _ppdLangString(lang, _("High")));
    cupsFilePuts(fp, "*CloseUI: *cupsPrintQuality\n");
  }
  ...
  if ((attr = ippFindAttribute(response, "job-presets-supported", IPP_TAG_BEGIN_COLLECTION)) != NULL)
  {
    for (i = 0, count = ippGetCount(attr); i < count; i ++)
    {
	  ...
	  */
      ...
      if ((localized_name = _ppdMessageLookup(strings, preset_name)) != preset_name)
  ...
  *buffer = '\0';
  ...
  set_error(_("Printer does not support required IPP attributes or document formats."), 1);
...
/*
 * 'ppd_get_url()' - Get a copy of the file at the given URL.
 */

static int				/* O  - 1 on success, 0 on failure */
ppd_get_url(http_t     **http,		/* IO - Current HTTP connection */
...
{
  _ppd_pwg_finishings_t	*f;		/* New finishings value */


  if ((f = (_ppd_pwg_finishings_t *)calloc(1, sizeof(_ppd_pwg_finishings_t))) != NULL)
  {
    f->value       = template;
...
ppd_pwg_compare_finishings(
    _ppd_pwg_finishings_t *a,		/* I - First finishings value */
    _ppd_pwg_finishings_t *b)		/* I - Second finishings value */
{
...
/*
 * 'ppd_pwg_compare_sizes()' - Compare two media sizes...
 */

static int				/* O - Result of comparison */
ppd_pwg_compare_sizes(cups_size_t *a,	/* I - First media size */
...
/*
 * 'ppd_pwg_copy_size()' - Copy a media size.
 */

static cups_size_t *			/* O - New media size */
ppd_pwg_copy_size(cups_size_t *size)	/* I - Media size to copy */
{
...
ppd_pwg_free_finishings(
    _ppd_pwg_finishings_t *f)		/* I - Finishings value */
{
