...
record_printer_options(const char *printer) {
  ...
  /* If there is a PPD file for this printer, either local for IPP network
     ...
     If the queue is pointing to one or more remote CUPS queues usually
     the PPD comes from the server or it is provided locally only
     if the implicitclass backend is used. In this case we cannot manage
     option settings in the PPD file, therefore we suffix the options
     with "-default" so that they go into /etc/cups/printers.conf when
     loaded next time 

     Discovered native IPP printers on the network have a permanent PPD
     generated by cups-browsed, so we can keep the PPD options in the PPD
     and do not add the "-default" suffix. */
  if ((ppdname = cupsGetPPD(printer)) == NULL) {
    ...
    ipp_printer =
      ((ppd_attr = ppdFindAttr(ppd, "NickName", NULL)) != NULL &&
       strstr(ppd_attr->value, "cups-filters") != NULL &&
       strncmp(ppd_attr->value, "Cluster of ", 11));
    ppdMarkDefaults(ppd);
                          ...
                          gpointer user_data)
{
  ...
  if ((ptr = strstr(text, " is now the default printer")) != NULL) {
	...
	if (!strcasecmp(p->name, printer) &&
	  ...
	  http = httpConnectEncrypt (p->ip ? p->ip : p->host, p->port,
...
gboolean handle_cups_queues(gpointer unused) {
  ...
  for (p = (remote_printer_t *)cupsArrayFirst(remote_printers);
	...
	debug_printf("Print queue %s has duplicates, using implicit class device URI %s\n",
		     p->name, device_uri);
	if (p->ppd == NULL && p->model == NULL && p->ifscript == NULL) {
	  ...
	  if ((http = httpConnectEncrypt(p->ip ? p->ip : p->host,
					 p->port ? p->port : ippPort(),
					 cupsEncryption())) == NULL) {
	  ...
	  if ((loadedppd = cupsGetPPD2(http, p->name)) == NULL &&
	    ...
	    debug_printf("Loading PPD file for printer %s from server %s:%d!\n",
			 p->name, p->host, p->port);
	    httpClose(http);
