        ...
        gpointer user_data)
{
  ...
  if (!strcasecmp(p->queue_name, printer) &&
    ...
    if(num_of_printers > 1){
      if (!supports_job_attributes_requested(printer,i,job_id,&print_quality)){
  ...
  if ((pdl_list = cupsArrayNew3((cups_array_func_t)strcasecmp, NULL, NULL, 0,
    ...
    /* Finding the best pdl supported by the printer, we need to send the
       document format to the implictclass backedn*/
    if(((attr = ippFindAttribute(printer_attributes, "document-format-supported", IPP_TAG_MIMETYPE)) != NULL) || (pdl && pdl[0] != '\0'))
    {
    ...
    if (cupsArrayFind(pdl_list, "application/vnd.cups-pdf")||cupsArrayFind(pdl_list, "application/pdf")) 
    ...
    /*Desiding the resolution to be sent with the job*/
    /* Finding the minimum and maximum resolution supported by the printer*/
    if(s && ((attr = ippFindAttribute(s->prattrs, "printer-resolution-supported", IPP_TAG_RESOLUTION)) != NULL)){
      for (i = 0, count = ippGetCount(attr); i < count; i ++){
        if ((res = ippResolutionToRes(attr, i)) != NULL){
          debug_printf("%d %d\n",res->x,res->y);
          if(i==0){
    ...
    /* If we are requesting normal print quality then send default resolution,
       for draft send minimum  resolution and for high, send the maximum resolution*/
    /* If none of the below dpi is selected then default dpi will be sent as 600*/
    ...
    if(s && print_quality == 3){
      if(min_res != NULL){
        if(min_res->x == min_res->y)
          ...
          snprintf(resolution,sizeof(resolution),"%dx%ddpi",min_res->x,min_res->y);
      }
      ...
      if(max_res != NULL){
        if(max_res->x == max_res->y)
          ...
          snprintf(resolution,sizeof(resolution),"%dx%ddpi",max_res->x,max_res->y);
      }
    }else if(s){
      if((attr = ippFindAttribute(s->prattrs, "printer-resolution-default", IPP_TAG_ZERO)) != NULL){
        if ((res = ippResolutionToRes(attr, 0)) != NULL){
          xres = res->x;
          yres = res->y;
          if(xres == yres)
...
gboolean update_cups_queues(gpointer unused) {
  ...
  if (IPPPrinterQueueType == PPD_YES) {
    ...
    if(num_cluster_printers==1){
      ...
      if((attr = ippFindAttribute(printer_attributes, "printer-make-and-model", IPP_TAG_TEXT)) != NULL)
      ...
      conflicts = generate_cluster_conflicts(p->queue_name,printer_attributes);
      debug_printf("Generated Constraints for queue %s\n",p->queue_name);
      sizes = get_cluster_sizes(p->queue_name);
      get_cluster_default_attributes(&printer_attributes,p->queue_name,default_pagesize,&default_color);
    ...
    if (ppdfile == NULL) {
      /* If we do not want CUPS-generated PPDs or we cannot obtain a
         ...
         ourselves */
      printer_ipp_response = (num_cluster_printers == 1) ? p->prattrs: printer_attributes; 
      if (!ppdCreateFromIPP(buffer, sizeof(buffer), printer_ipp_response,make_model,
          pdl, color, duplex,conflicts,sizes,default_pagesize,default_color)) {
        if (errno != 0)
